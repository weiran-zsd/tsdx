...
const fixtureName = 'build-withConfig-defineConfig';
const stageName = `stage-integration-${fixtureName}`;

describe('integration :: dts build :: dts-config.js :: defineConfig', () => {
  beforeAll(() => {
  ...
  it('should create a CSS file in the dist/ directory', () => {
    ...
    expect(
      shell.test('-f', 'dist/build-withconfig-defineconfig.cjs.development.css')
    );
  ...
  it('should autoprefix and minify the CSS file', async () => {
    ...
    const cssText = await fs.readFile(
      './dist/build-withconfig-defineconfig.cjs.development.css'
    );
  ...
  it('should compile files into a dist directory', () => {
    ...
    expect(
      shell.test('-f', 'dist/build-withconfig-defineconfig.cjs.development.js')
    ).toBeTruthy();
    expect(
      shell.test(
        '-f',
        'dist/build-withconfig-defineconfig.cjs.production.min.js'
      )
    ).toBeTruthy();
    expect(
      shell.test('-f', 'dist/build-withconfig-defineconfig.esm.js')
    ).toBeTruthy();
